...
const PREFS_TEMPLATE = `# SheepShaver preferences - generated by web UI
...
# Video codec for web streaming (png, h264, or av1)
webcodec {{WEBCODEC}}
...
function parsePrefsFile(content) {
    ...
    for (const line of lines) {
        ...
        switch (key) {
            case 'rom':
                // Store just the filename (strip any path prefix - relative or absolute)
                // e.g., "/path/to/storage/roms/MacOSROM.rom" -> "MacOSROM.rom"
                // e.g., "storage/roms/MacOSROM.rom" -> "MacOSROM.rom"
                config.rom = value.replace(/^.*storage\/roms\//, '').replace(/^.*\//, '');
                // If no slashes were in the original, keep as-is
                if (!value.includes('/')) config.rom = value;
                break;
            case 'disk':
                // Store just the filename
                let diskFile = value.replace(/^.*storage\/images\//, '').replace(/^.*\//, '');
                if (!value.includes('/')) diskFile = value;
                config.disks.push(diskFile);
                break;
            case 'cdrom':
                // Store just the filename
                let cdromFile = value.replace(/^.*storage\/images\//, '').replace(/^.*\//, '');
                if (!value.includes('/')) cdromFile = value;
                config.cdroms.push(cdromFile);
                break;
            case 'ramsize':
...
function generatePrefsFile(config, romsPath, imagesPath) {
    ...
    let prefs = PREFS_TEMPLATE
        ...
        .replace('{{IDLEWAIT}}', config.idlewait ? 'true' : 'false')
...
function onRomChange() {
    // SheepShaver uses a single ROM type, no model auto-detection needed
    const romName = document.getElementById('cfg-rom')?.value;
    if (romName) {
        currentConfig.rom = romName;
    }
}
...
function updateConfigUI() {
    ...
    if (idlewaitEl) idlewaitEl.checked = currentConfig.idlewait;
...
async function saveConfig() {
    ...
    currentConfig.ram = parseInt(document.getElementById('cfg-ram')?.value || 1024);
